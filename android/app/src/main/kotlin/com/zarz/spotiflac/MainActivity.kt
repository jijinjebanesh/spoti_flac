...
class MainActivity: FlutterFragmentActivity() {
    ...
    // Store FlutterEngine for audio_service plugin access
    ...
    override fun getFlutterEngine(): FlutterEngine? = cachedFlutterEngine ?: super.getFlutterEngine()
    ...
    companion object {
        ...
        // Static reference to FlutterEngine for audio_service plugin access
        @Volatile
        private var instance: MainActivity? = null
        
        fun getMainActivity(): MainActivity? = instance
    ...
    /**
     * Return the cached engine ID so the FlutterFragment reuses the engine
     * we pre-created in onCreate() rather than creating a new one.
     */
    override fun getCachedEngineId(): String = "audio_service_engine"

    override fun onCreate(savedInstanceState: Bundle?) {
        // Pre-create and cache the FlutterEngine BEFORE super.onCreate().
        //
        // Lifecycle order inside FlutterFragmentActivity.super.onCreate():
        //   1. FlutterFragment is created and committed
        //   2. Fragment.onAttach() -> delegate.onAttach():
        //      a. setupFlutterEngine()  – finds our engine via getCachedEngineId()
        //      b. attachToActivity()    – triggers audio_service's onAttachedToActivity,
        //         which calls AudioServicePlugin.getFlutterEngine(activity) that looks up
        //         FlutterEngineCache.get("audio_service_engine").  If missing, it creates
        //         a *separate* engine whose DartExecutor differs from the plugin binding's
        //         messenger -> wrongEngineDetected = true -> IllegalStateException.
        //      c. configureFlutterEngine() – our method-channel setup (too late to cache)
        //
        // By creating the engine here and caching it with the key the plugin expects,
        // both the fragment (via getCachedEngineId) and the plugin (via FlutterEngineCache)
        // resolve to the exact same FlutterEngine instance.
        if (FlutterEngineCache.getInstance().get("audio_service_engine") == null) {
            val dartVmArgs = mutableListOf<String>()
            // Apply the same Impeller detection that ImpellerAwareFlutterFragment uses
            if (shouldDisableImpeller()) {
                android.util.Log.w("SpotiFLAC", "Pre-warming engine with Impeller disabled for ${Build.MODEL}")
                dartVmArgs.add("--enable-impeller=false")
            }
            // Include any intent-based shell args (e.g. --enable-dart-profiling from CLI)
            try {
                val intentArgs = FlutterShellArgs.fromIntent(intent)
                dartVmArgs.addAll(intentArgs.toArray())
            } catch (_: Exception) {}

            val engine = FlutterEngine(this, dartVmArgs.toTypedArray())
            engine.dartExecutor.executeDartEntrypoint(
                DartExecutor.DartEntrypoint.createDefault()
            )
            FlutterEngineCache.getInstance().put("audio_service_engine", engine)
            android.util.Log.i("SpotiFLAC", "Pre-warmed FlutterEngine cached as audio_service_engine")
        }
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        // Keep a local reference for getFlutterEngine() override
