...
class QueueTab extends ConsumerStatefulWidget {
  ...
  final double miniPlayerBottomInset;
...
class _QueueTabState extends ConsumerState<QueueTab> {
  ...
  Future<void> _playUnifiedItems(
    ...
    final target = items[startIndex.clamp(0, items.length - 1)];
  ...
  Future<void> _playRecentDownload(
    DownloadHistoryItem item, {
    required List<DownloadHistoryItem> items,
    required int index,
  }) async {
    try {
  ...
  Widget build(BuildContext context) {
    ...
    final currentLoadedCount = switch (historyFilterMode) {
      ...
      _ => currentPageData.items.length,
    };
    ...
    final bottomPadding =
        MediaQuery.paddingOf(context).bottom + widget.miniPlayerBottomInset;
          ...
          ScrollConfiguration(
            ...
            child: NestedScrollView(
              headerSliverBuilder: (context, innerBoxIsScrolled) => [
                ...
                _buildRecentDownloadsSliver(context, colorScheme),
              ...
              body: PageView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final pageRaw = pageData(filterMode);
                  return _buildFilterContent(
                    ...
                    pageLocalItems: pageRaw.localItems,
                    pageHistoryItems: pageRaw.historyItems,
                  );
                },
              ),
            ),
  ...
  Widget _buildRecentDownloadsSliver(
    BuildContext context,
    ColorScheme colorScheme,
  ) {
    return Consumer(
      builder: (context, ref, child) {
        final items = ref.watch(libraryRecentDownloadsProvider);
        if (items.isEmpty) {
          return const SliverToBoxAdapter(child: SizedBox.shrink());
        }

        final coverSize = _queueCoverSize() * 1.6;
        final rowHeight = coverSize + 28;
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Semantics(
                        ...
                        child: GestureDetector(
                          onTap: () => _playRecentDownload(
                            item,
                            items: items,
                            index: index,
                          ),
                          onLongPress: () => _navigateToHistoryMetadataScreen(
                            item,
  ...
  Widget _buildHistoryCoverImage(
    DownloadHistoryItem item,
    ColorScheme colorScheme,
    double size,
  ) {
    return ClipRRect(
      ...
      child: _buildUnifiedCoverImage(
        UnifiedLibraryItem.fromDownloadHistory(item),
        colorScheme,
        size,
      ),
    );
  ...
  Widget _buildDeviceFoldersFilterContent({
    required BuildContext context,
    required ColorScheme colorScheme,
  ...
  Widget _buildPlaylistsFilterContent({
    required BuildContext context,
    required ColorScheme colorScheme,
    required String historyViewMode,
    required LibraryCollectionsState collectionState,
    ...
    if (isPageLoading && playlists.isEmpty) {
      return const Center(child: CircularProgressIndicator());
    }
        ...
        slivers: [
          SliverFillRemaining(
            ...
            child: Column(
              ...
              children: [
                _buildEmptyState(context, colorScheme, 'playlists'),
                const SizedBox(height: 16),
                FilledButton.icon(
  ...
  Widget _buildFilterContent({
    ...
    required bool hasMoreLibrary,
    required bool isPageLoading,
    required List<LocalLibraryItem> pageLocalItems,
    required List<DownloadHistoryItem> pageHistoryItems,
  }) {
    if (filterMode == 'folders') {
      return _buildDeviceFoldersFilterContent(
        context: context,
        colorScheme: colorScheme,
        historyViewMode: historyViewMode,
    ...
    if (filterMode == 'playlists') {
      return _buildPlaylistsFilterContent(
        context: context,
        colorScheme: colorScheme,
        historyViewMode: historyViewMode,
        collectionState: collectionState,
                        ...
                        child: LongPressDraggable<UnifiedLibraryItem>(
                          ...
                          childWhenDragging: Opacity(
                            ...
                            child: _buildUnifiedGridItem(
                              ...
                              playbackQueue: filteredUnifiedItems,
                              playbackStartIndex: trackIndex,
                            ),
                          ),
                          child: _buildUnifiedGridItem(
                            context,
          ...
          child: InkWell(
            onTap: isCompleted
                ? () async {
                    ...
                    for (final h in ref.read(downloadHistoryProvider).items) {
                      if (h.filePath == item.filePath) {
                        historyItem = h;
                        break;
                      }
                    }
                    if (historyItem != null) {
                      await _playRecentDownload(
                        historyItem,
                        items: [historyItem],
                        index: 0,
                      );
                    } else {
                      await _navigateToMetadataScreen(item);
                    }
                  }
                : null,
  ...
  Widget _buildUnifiedLibraryItem(
    ...
    required List<UnifiedLibraryItem> playbackQueue,
    required int playbackStartIndex,
  }) {
